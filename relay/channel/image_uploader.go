...
func ExtractMultipartFilesFromMultipart(c *gin.Context, fieldNames []string) ([]*multipart.FileHeader, error) {
	var filesOut []*multipart.FileHeader
	if c.ContentType() != gin.MIMEMultipartPOSTForm {
		return filesOut, nil
	}
	if _, err := c.MultipartForm(); err != nil {
		return nil, fmt.Errorf("failed to parse multipart form request: %w", err)
	}
	...
	for _, fieldName := range fieldNames {
		if files, found := mf.File[fieldName]; found && len(files) > 0 {
			filesOut = append(filesOut, files...)
		}
		// Also try array notation like "image[]"
		if files, found := mf.File[fieldName+"[]"]; found && len(files) > 0 {
			filesOut = append(filesOut, files...)
		}
		// Try indexed notation like "image[0]", "image[1]", etc.
		for indexedFieldName, files := range mf.File {
			if strings.HasPrefix(indexedFieldName, fieldName+"[") && len(files) > 0 {
				filesOut = append(filesOut, files...)
			}
		}
	}
...
func ExtractImageFilesFromMultipart(c *gin.Context, fieldNames []string) ([]*multipart.FileHeader, error) {
	return ExtractMultipartFilesFromMultipart(c, fieldNames)
}

func TryConvertMultipartFileFieldsToURLs(c *gin.Context, fieldNames []string, options ImageUploadOptions) ([]string, error) {
	files, err := ExtractMultipartFilesFromMultipart(c, fieldNames)
	if err != nil {
		return nil, err
	}
	...
	for _, file := range files {
		fileURL, err := UploadMultipartFile(c, file, userID, options)
		if err != nil {
			return nil, err
		}
...
func TryConvertMultipartFieldsToURLs(c *gin.Context, fieldNames []string, options ImageUploadOptions) ([]string, error) {
	if c.ContentType() != gin.MIMEMultipartPOSTForm {
		return nil, nil
	}
	if _, err := c.MultipartForm(); err != nil {
	...
	if c.Request.MultipartForm != nil && c.Request.MultipartForm.Value != nil {
		for _, fieldName := range fieldNames {
			for _, val := range c.Request.MultipartForm.Value[fieldName] {
	...
	fileURLs, err := TryConvertMultipartFileFieldsToURLs(c, fieldNames, options)
	if err != nil {
		return nil, err
	}
...
func TryConvertMultipartSingleFileFieldToURL(c *gin.Context, currentValue string, fieldNames []string, options ImageUploadOptions) (string, bool, error) {
	if strings.TrimSpace(currentValue) != "" || c.ContentType() != gin.MIMEMultipartPOSTForm {
	...
	fileURL, err := UploadMultipartFile(c, files[0], GetUserIDFromContext(c), options)
	if err != nil {
		return "", false, err
	}
